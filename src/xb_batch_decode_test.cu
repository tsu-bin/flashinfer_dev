...
void _TestBatchDecodingKernelCorrectness() {
  ...
  // const size_t num_qo_heads = 128;
  // const size_t head_dim = 512;

  const size_t num_qo_heads = 4;
  ...
  std::vector<int32_t> seq_lens{950, 11, 70, 17, 999};

  flashinfer::PosEncodingMode pos_encoding_mode = flashinfer::PosEncodingMode::kRoPELlama;
  ...
  for (size_t i = 0; i < batch_size; ++i) {
    ...
    // std::vector<DTypeKV> vi(seq_len * num_kv_heads * head_dim);
    // utils::vec_normal_(vi);
    auto& vi = ki;

    std::vector<DTypeQO> o_ref_i = cpu_reference::single_mha<DTypeQO, DTypeKV, DTypeQO>( // compute reference output
        qi, ki, vi, 1, seq_len, num_qo_heads, num_kv_heads, head_dim, false, QKVLayout::kNHD,
        pos_encoding_mode);
    // std::vector<DTypeQO> o_ref_i(num_qo_heads*head_dim);
    keys.push_back(ki);
    values.push_back(vi);
  ...
  flashinfer::BatchDecodeHandlerPlan<DTypeQO, DTypeKV, DTypeQO, int32_t>(
...
TEST(FlashInferCorrectnessTest, XB_BatchDecodeKernelCorrectnessTest) {
  _TestBatchDecodingKernelCorrectness();
}
